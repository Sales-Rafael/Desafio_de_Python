...
df_credit_card_agrupado = df_credit_card.groupby(['Card Type', 'Exp Type'])[['Amount']].sum().reset_index()
...
with st.sidebar:
    ...
    selected_card = st.selectbox("Tipo de Cartão", df_credit_card_agrupado["Card Type"].unique(),index=0)
    df_card_filtered = df_credit_card_agrupado[df_credit_card_agrupado["Card Type"] == selected_card]
...
col_3 = st.columns((1,1,1), gap='medium')
...
with col_3[0]:
    fig = px.pie(df_card_filtered, values='Amount', names='Exp Type', title ="Gasto com cartão de credito(Por tipo de cartão)",color_discrete_sequence=rainbow)
    fig.update_traces(
...
with col_3[1]:
    ...
    st.plotly_chart(fig) 
with col_3[2]:
    fig_pie = px.pie(Tabela_carros_agrupado, values='Emissão de CO2(g/km)', names='Fabricante', title ="Emissão de CO2 por fabricante de carro",color_discrete_sequence=rainbow)
